...
(function () {
  'use strict';
  const C=window.WholeheartedCore, main=document.getElementById('main');
  ...
  function render(){
    ...
    document.title=(match?'Prayer · ':'')+'Wholehearted · W.H.E.M.S. Prayer';focus();
  }
  ...
  function download(data,name,type='text/plain;charset=utf-8'){
    const url=URL.createObjectURL(new Blob([data],{type}));const a=document.createElement('a');a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),2000);notify('Download prepared. Keep personal prayer files private.');
  }
  ...
  document.addEventListener('click',async event=>{
    ...
    try {
      ...
      if(action==='save'){
        ...
        journal=result;dirty=false;const savedId=draft.id;draft=null;go('entry-'+savedId);return notify('Saved in this browser. No cloud copy was created.');
      }
      if(action==='migrate'){
        if(!journal.ok)return notify(journal.error);
